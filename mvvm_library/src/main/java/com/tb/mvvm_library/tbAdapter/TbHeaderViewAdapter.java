 ...
 * 使其具备给列表添加头部({@link TbHeaderViewAdapter#addHeaderView(View)})
 * 和添加尾部({@link TbHeaderViewAdapter#addFooterView(View)})的功能。
 ...
 * Dat:2017/11/10
 */
public class TbHeaderViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public TbHeaderViewAdapter(RecyclerView.Adapter adapter) {
        mAdapter = adapter;
        if (mAdapter != null) {
            //注册mAdapter的数据变化监听
    ...
    public void setAdapter(RecyclerView.Adapter adapter) {
        if (adapter instanceof TbHeaderViewAdapter) {
